  ...
  void _showEditDialog(Post post) async {
    ...
    if (updatedContent != null && updatedContent.isNotEmpty) {
      try {
        ...
        await repository.editPost(post.id, updatedContent);
        final updatedPost = Post(
          id: post.id, authorId: post.authorId, authorName: post.authorName,
          authorPic: post.authorPic, content: updatedContent, mediaUrls: post.mediaUrls,
          likes: post.likes, comments: post.comments, timestamp: post.timestamp,
        );
        ref.read(feedProvider.notifier).updatePost(updatedPost);
      } catch (e) {
